...
class ArmEspService {
    static async obtenerTodos() {
        try {
            ...
            Notificacion.error('No se pudieron cargar los datos. Ver consola para más detalles.');
            return [];
        }
    }
    ...
    static async guardar(armEsp) {
        try {
            ...
            if (!datosAEnviar.abreviatura || !datosAEnviar.armesp_completo || !datosAEnviar.tipo) {
                const errorMsg = 'Faltan datos requeridos: ' + 
            ...
            if (datosAEnviar.tipo !== 'Arma' && datosAEnviar.tipo !== 'Especialidad') {
            ...
            if (!response.ok) {
                ...
                const errorMsg = errorData.message || 
                               (errorData.errors ? JSON.stringify(errorData.errors) : `Error ${response.status}: ${response.statusText}`);
                throw new Error(errorMsg);
            }
    ...
    static async eliminar(id) {
        try {
            console.log(`Enviando solicitud para eliminar arma/especialidad con ID: ${id}`);
...
class ArmEspUI {
    static inicializar() {
        try {
            ...
            armEspModal = new bootstrap.Modal(document.getElementById('agregarArmEspModal'));
    ...
    static async cargarDatos() {
        try {
            ...
            console.error('Error al cargar datos:', error);
            Notificacion.error('Error al cargar los datos. Ver consola para más detalles.');
        }
    }
    ...
    static crearFilaTabla(armEsp) {
        ...
        const nombreCompleto = armEsp.armEspCompleto || armEsp.armesp_completo || '';
        ...
        if (tipo === null || tipo === undefined || tipo === '') {
            console.warn(`El registro con ID ${id} no tiene tipo definido. Se usará 'Arma' por defecto.`, armEsp);
            tipo = 'Arma';
        }
        
        // Asegurarse de que el tipo sea 'Arma' o 'Especialidad'
        if (tipo !== 'Arma' && tipo !== 'Especialidad') {
            console.warn(`Tipo no válido: "${tipo}" para el registro con ID ${id}. Se usará 'Arma' por defecto.`, armEsp);
            tipo = 'Arma';
        }
    ...
    static obtenerDatosFormulario() {
        try {
            ...
            const nombreCompleto = descripcionInput.value ? descripcionInput.value.trim() : ''; 
    ...
    static async guardar() {
        try {
            ...
            if (!formData.tipo) {
                Notificacion.advertencia('Por favor seleccione un tipo (Arma o Especialidad)');
                return false;
            }
            ...
            if (formData.abreviatura.trim().length > 20) {
                Notificacion.advertencia('La abreviatura no puede tener más de 20 caracteres');
                return false;
            }
            
            // Validar longitud del nombre completo
            if (formData.armesp_completo.trim().length > 100) {
                Notificacion.advertencia('El nombre completo no puede tener más de 100 caracteres');
                return false;
            }
            ...
            try {
                ...
                const mensaje = error.message || 'Error al guardar los datos. Por favor, intente nuevamente.';
            ...
            // Mostrar mensaje de error
            const mensaje = error.message || 'Error en el proceso de guardado. Por favor, intente nuevamente.';
            Notificacion.error(mensaje);
    ...
    static configurarEventos() {
        try {
            ...
            const btnAgregar = document.querySelector('[data-bs-target="#agregarArmEspModal"]');
            if (btnAgregar) {
